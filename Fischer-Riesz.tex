...
et on appelle $\Lpc(m)$ l'ensemble de toutes les fonctions à valeurs complexes pour lesquelles :
\[ \normp{f} < \infty . \]
D'après ce qui précède, $\normp{\cdot}$ est une semi-norme sur l'ensemble $\Lpc(m)$.
\end{defn}

\begin{lem}
Les espaces $\Lpc(m)$ pour $1 \leq p \leq \infty$ sont des \Cev.
\end{lem}

\begin{rmq}
On remarque que $\normp{f} = \normp{g} \Rightarrow f =g  \pp$. Ainsi la norme n'est pas définie.
Néanmoins, on démontre facilement que la relation \og sont égaux presque partout \fg{} définie une relation d'équivalence sur 
$\Lpc(m)$. Ainsi, en introduisant l'espace $\Lp(m)$ comme l'espace quotient de $\Lpc(m)$ par cette relation d'équivalence, 
on obtient un espace vectoriel normé.
\end{rmq}
