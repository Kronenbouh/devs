...
\section{Le processus de Galton-Watson}
...
On suppose que tous les $X_i^n$ admettent un moment d'ordre 2 , et on note :
\[ m = E[X] \et \sigma_X = \var(X) . \] 
On pose :
...
$\set{Z_n = 0} ; n \geq 2$ soit, par $\sigma$-additivité :
...
\begin{lem}[identité de Wald]
\label{lem:Wald}
Soient $(X_n)_{n \geq 1}$ une suite de variables aléatoire indépendantes, identiquement distribuées et à valeurs dans $\N$ 
et $N$ une variable aléatoire à valeurs dans $\N$. On suppose que les variables $X_i$ et $N$ sont mutuellement indépendantes.
On définit la nouvelle variable aléatoire $S_N = X_1 + \dots + X_N$.
On note $G_N$ la fonction génératrice de $N$, $G_X$ celle, commune, des variables $X_i$, et $G_S$ celle de $S_N$.
On a alors $G_S = G_N \circ G_X$, i.e. :
\[ \forall t \in [-1, 1] \quad G_S(t) = G_N(G_X(t)) . \]
\end{lem}

\begin{proof}
Par indépendance des $X_i$, on a :
\[ G_{S_n} = E[t^S_n] = E[(t^X)^n] = (G_X)^n . \]
La probabilité que $S_N$ soit égale à un entier $n$ est :
\begin{align*}
P\set{S_N = n} & = \sum_{k=0}^{\infty} P\set{S_N = n, N=k} \\
& = \sum_{k=0}^{\infty} P\set{S_k = n, N=k} \\
& = \sum_{k=0}^{\infty} P\set{S_k = n} P\set{ N=k},
\end{align*}
Par indépendance des variables $X_i$ avec $N$. Donc, pour $t \in [-1, 1]$ :
\begin{align*}
G_S(t) & = \sum_{n=0}^{\infty} P \set{S_N = n} t^n  = \sum_{n=0}^{\infty}  \sum_{k=0}^{\infty} P\set{S_k = n} P\set{ N=k} t^n \\
& = \sum_{k=0}^{\infty} \sum_{n=0}^{\infty} P\set{S_k = n} P\set{N=k} t^n \qquad \text{(Fubini)} \\
& = \sum_{k=0}^{\infty} P\set{N=k} \sum_{n=0}^{\infty} P\set{S_k = n} t^n = \sum_{k=0}^{\infty} P\set{N=k} G_{S_k}(t) \\
& = \sum_{k=0}^{\infty} P\set{N=k} (G_X(t))^k = G_N(G_X(t))
\end{align*}
\end{proof}
...
\begin{proof}
On rappelle que $Z_{n+1} = \sum_{i=1}^{Z_n} X_i^n$ et les variables $X_i^n$ étant indépendantes, on a, pour $n$ fixé, 
indépendance entre la variable $Z_n$ et les varibales $X_i^n$. On peut donc appliquer l'identité de Wald (cf. $\ref{lem:Wald}$) dont 
...
$\forall x \in [0, a[, G(x) > x$ (sinon $G-\id$ s'annule par continuité en un point $x_0<a$). \\
...
\subsection{Extinction de la population}
...
\begin{enumerate}
\item \textbf{$m<1$}, on dit que le système est sous-critique. Pout tout $t \in [0,1[, f(t) \leq m$ donc $G(t) - 1 \geq m(t-1)$ 
...
\item sinon $p_0 + p_1 < 1$ et il existe $k \geq 2$ tel que $p_k > 0$. Or, pour tout $t \in ]0,1[$ :
\[ G''(t) = \sum_{k=2}^{\infty}k(k-1)p_kx^{k-2} , \]
ce qui implique que la fonction $G$ est strictement convexe sur $]0,1[$ en particulier $f$ est strictement croissante sur $]0,1[$. \\
Donc $\forall t \in [0,1[, G(t) - t > 0$ et 1 est le seul point fixe de $G$. La population s'éteind presque sûrement.
\end{enumerate}

\item \textbf{$m>1$} on dit que le système est sur-critique. Par continuité de $f$, on peut trouver $t_0 \in [0,1[$ tel que 
...
\subsection{Evolution de la population}
...
\begin{lem}
Le rayon de convergence de la série $G_n$ est au moins égal à 1 et $G$ est de classe $\C^{\infty}$ sur $]-1,1[$. \\
En particulier, $G$ est croissante et convexe sur $[0,1]$.
\end{lem}

\begin{thm}[un théorème de Fubini]
