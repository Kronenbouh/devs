...
Dans cette section, $\K$ est un corps et $E$ un \Kev de dimension finie $n$.
...
\[ x = g_1 + \dots + g_q, \]
avec $g_j \in G_j$ pour $1 \leq j \leq q$. Or $f(x) = \lambda_i x = f(g_1) + \dots + f(g_q)$, on a donc :
\[ x = \dfrac1{\lambda_i} f(g_1) + \dots + \dfrac1{\lambda_i} f(g_q), \]
avec $f(g_j) \in G_j$ pour $1 \leq j \leq q$ par stabilité des $G_j$ par $f$.
