...
Dans cette section $E$ est un espace euclidien (de dimension $n \geq 1$) muni d'un produit scalaire dénoté par $\angl{.}$.

\begin{defn}[Endomorphisme autoadjoint]
Un endomorphisme $f$ de $E$ est dit autoadjoint si :
\[ \forall x,y \in E, \angl{f(x), y} = \angl{x, f(y)} . \]
\end{defn}

\begin{lem}
\label{lem:1}
Un endomorphisme est autoadjoint si, et seulement si, sa matrice dans une base orthonormée est symétrique.
\end{lem}
